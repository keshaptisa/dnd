...
def text_to_speech(text):  #озвучка текста от гпт
...
async def action(update: Update, context: ContextTypes.DEFAULT_TYPE):   #эту хуету писала гпт вообще не факт что работает но расскажу как должна
    ...
    await perform_action(character, action_text, update) #обращаемся к фунеции которая дает нам необходимый результат и лстальную хуету
